...
import com.fasterxml.jackson.databind.{JsonMappingException, ObjectMapper, RuntimeJsonMappingException}
import com.fasterxml.jackson.databind.json.JsonMapper
import com.fasterxml.jackson.module.scala.DefaultScalaModule

private[processes] object Lshw:
  given jsonMapper: ObjectMapper =
  ...
  def readDisks(`class`: String = "disk")(using ProcessConfig)(using logger: HWLLoggable): Option[Seq[CdRom]] =
    ...
    if json == "" then
      ...
      try
        jsonMapper.readValue(json, classOf[Array[Device]])
          .collect:
        ...
        case e: JsonMappingException =>
          logger().error(s"CD deserialisation error: ${e.getMessage}")
          None
        case _ => None
      end try
  end readDisks
