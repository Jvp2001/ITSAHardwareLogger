...
class HDSentinelReader(using HWLLoggable):
  ...
  def getAllNodesInElementsStartingWith[T : ClassTag](startingWith: HDSentinelReader.ElementName, childName: HDSentinelReader.ElementName)(using HWLLoggable): Seq[T] =
...
object HDSentinelReader:
  ...
  def apply[T: ClassTag]()(using config: ProcessConfig, logger: HWLLoggable): HDSentinelReader =
  ...
  inline def apply[T: ClassTag](elem: Elem)(using HWLLoggable): HDSentinelReader =
    val reader = new HDSentinelReader
    reader.read(elem)
    reader
