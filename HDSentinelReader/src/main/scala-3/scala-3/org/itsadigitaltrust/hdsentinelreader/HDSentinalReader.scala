...
class HDSentinalReader(using HWLLoggable):
  ...
  def getAllNodesInElementsStartingWith[T : ClassTag](startingWith: HDSentinalReader.ElementName, childName: HDSentinalReader.ElementName)(using HWLLoggable): Seq[T] =
    given xmlMapper:XmlMapper = xmlParser.xmlMapper
    val nodes = xmlParser.getAllNodesStartingWith(startingWith)
    nodes.map: node =>
      node \\> childName

end HDSentinalReader

object HDSentinalReader:
  ...
  def apply[T: ClassTag]()(using config: ProcessConfig, logger: HWLLoggable): HDSentinalReader =
    val reader = new HDSentinalReader
    if OSUtils.onLinux then
      val xml = sudo"HDSentinal -r -xml -dump"
      logger().info(s"XML: $xml")
      reader.read(xml)
    reader

  inline def apply[T: ClassTag](elem: Elem)(using HWLLoggable): HDSentinalReader =
    val reader = new HDSentinalReader
    reader.read(elem)
    reader
