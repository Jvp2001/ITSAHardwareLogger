...
final class GitHubIssueReporter(file: Option[String])(using default: IssueReporterProps) extends IssueReporter:
  ...
  override def report(issue: ReportedIssue): IssueReportStatus =
    ...
    Try(gitIssue.create())
      .fold(
        ex => IssueReportStatus.Error(ex.getMessage |> Option[String]),
        _ => IssueReportStatus.Success
      )
